...
ELSE
	...
	FOR i = 3 TO (nlinhas - 3)
		...
		IF SUBSTR(linha, 14, 1) = 'T'
			ret_sub = SUBSTR(linha, 214, 2)
		ENDIF
		SELECT tarifas
		SEEK '001' + ret_cod + ret_sub
		IF FOUND()
			IF SUBSTR(linha, 14, 1) = 'T'
				ret_id = INT(VAL(SUBSTR(linha, 45, 10)))
			...
			IF SUBSTR(linha, 14, 1) = 'U'
				...
				IF FOUND()
					...
					ENDIF
					SELECT rectar_2
					SEEK STR(ret_id, 8) + '001' + ret_cod + ret_sub + DTOC(ret_dpg)
					IF  .NOT. FOUND()
						SELECT rectarif
						GOTO BOTTOM
						ult_id = rectarif.id
						...
						REPLACE rectarif.banco_id WITH '001'
						REPLACE rectarif.tarifa_id WITH ret_cod
						REPLACE rectarif.tarsub_id WITH ret_sub
